...
- Keep common commands shallow and resource-first. Prefer `pccli songs list` over mirroring API or product taxonomy like `pccli services library songs list`.
- Add product or namespace layers only when they resolve a real user-facing ambiguity.
...
- If the banner changes, update `scripts/banner.ansi`, preview it with `scripts/banner-mockup.sh`, run the banner sync task, and do not commit generated binaries from `bin/`.
