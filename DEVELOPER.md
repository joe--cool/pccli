...
- `scripts/banner-mockup.sh`: preview helper for the banner asset.
...
Follow the shape used by mature resource-oriented CLIs: keep common workflows resource-first and shallow. Prefer `pccli songs list` over `pccli services library songs list`; the command should match how users describe the work, not Planning Center's internal product hierarchy.

Use this decision order:

1. Start with the user-facing resource or workflow noun: `songs`, `people`, `plans`.
2. Put the action next: `list`, `show`, `create`, `update`, `archive`.
3. Use flags for filters and context instead of extra hierarchy.
4. Add a namespace only when two resources would otherwise collide or when users naturally distinguish the workflows that way.
5. Keep aliases out until there is released usage to preserve.
...
When changing the banner, edit `scripts/banner.ansi`, preview it with `scripts/banner-mockup.sh`, then run:
...
The quick start should stay setup-first as the CLI grows: install, authenticate, confirm the CLI runs, then choose the currently relevant workflow. Do not make one feature area look like the permanent entry point for the whole product.
